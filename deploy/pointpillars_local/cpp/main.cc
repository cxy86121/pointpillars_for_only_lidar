...
///run
// ./build/main
// --model_file /paddle/MyCode/Paddle3D/output_model/pointpillars.pdmodel
// --params_file /paddle/MyCode/Paddle3D/output_model/pointpillars.pdiparams
// --lidar_file /paddle/MyCode/mmdetection3d/mytest/test_200_bin/440.bin
...
DEFINE_string(point_cloud_range, "0 -39.68 -3 69.12 39.68 1",
              "Range of point cloud for voxelize operation");
DEFINE_string(voxel_size, ".16 .16 4", "Size of voxels for voxelize operation");
DEFINE_int32(max_points_in_voxel, 32, "Maximum number of points in a voxel"); //100
DEFINE_int32(max_voxel_num, 12000, "Maximum number of voxels");
DEFINE_int32(gpu_id, 0, "GPU card id");
DEFINE_int32(use_trt, 1,
...
DEFINE_string(dynamic_shape_file, "/paddle/MyCode/Paddle3D/paddle_inference/model_output/shape_info.txt",
...
bool parse_result(const std::vector<float> &box3d_lidar,
                  ...
                  const std::vector<float> &scores, string save_file, bool save_flag) {
  int num_bbox3d = scores.size();
  int bbox3d_dims = box3d_lidar.size() / num_bbox3d;

  ofstream box_outfile;
  box_outfile.open(save_file, ios::out|ios::app);
  ...
  for (size_t box_idx = 0; box_idx != num_bbox3d; ++box_idx) {
    ...
    if(scores[box_idx]>0.3){
        ...
        if (bbox3d_dims == 7) {
          ...
          if(save_flag){
              box_outfile<<box3d_lidar[box_idx * 7 + 0]<<" "<<box3d_lidar[box_idx * 7 + 1]<<" "<<box3d_lidar[box_idx * 7 + 2]
              <<" "<<box3d_lidar[box_idx * 7 + 3]<<" "<<box3d_lidar[box_idx * 7 + 4]<<" "<<box3d_lidar[box_idx * 7 + 5]<<" "
              <<box3d_lidar[box_idx * 7 + 6]<<endl;
          }
      }
    }
  }
...
//  }
  ...
  FLAGS_model_file="/paddle/MyCode/Paddle3D/paddle_inference/model_output/pointpillars.pdmodel";
  FLAGS_params_file="/paddle/MyCode/Paddle3D/paddle_inference/model_output/pointpillars.pdiparams";
//
//  FLAGS_model_file="/paddle/MyCode/Paddle3D/paddle_inference/model_output_person/pointpillars.pdmodel";
//  FLAGS_params_file="/paddle/MyCode/Paddle3D/paddle_inference/model_output_person/pointpillars.pdiparams";

  std::string input_dir = "/paddle/MyCode/TEST/M1/M1_BIN/";
  std::string save_dir = "/paddle/MyCode/TEST/M1/test_result/";

//  std::string input_dir = "/paddle/MyCode/TEST/BX/200m_bin/";
//  std::string save_dir = "/paddle/MyCode/TEST/BX/test_result/";

  FLAGS_point_cloud_range="0 -39.68 -3 69.12 39.68 1";
  FLAGS_voxel_size=".16 .16 4";
  FLAGS_max_points_in_voxel=32;
  FLAGS_max_voxel_num=40000;
...
//  }
  ...
  for(int i=0;i<1694;++i){
    FLAGS_lidar_file=input_dir+to_string(i)+".bin";
    ifstream pcd_file(FLAGS_lidar_file.c_str());
    if (!pcd_file)
    {
//        cout<<FLAGS_lidar_file.c_str()<<" not find"<<endl;
        continue;
    }

    LOG(INFO) << "bin_file: " << FLAGS_lidar_file << " ";
    ...
    middle = clock();
    double preprocess_cost = (double)(middle - start) / CLOCKS_PER_SEC;
    printf( "preprocess cost time:  %f seconds\n", preprocess_cost);
    ...
    bool save_flag = true;
    string save_file=save_dir+to_string(i)+".txt";

    parse_result(box3d_lidar, label_preds, scores, save_file, save_flag);
  }
